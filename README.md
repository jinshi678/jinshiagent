...
| FastAPI 服务 | 提供 HTTP API | 将 Agent 封装为 POST /chat 接口 |
| Docker 容器 | 生产部署、微服务 | 编写 Dockerfile，多阶段构建 |
| 云函数 | 按需调用、低成本 | 适配 AWS Lambda / 腾讯云函数 |
| 定时任务 | 周期性任务 | 使用 cron 或 Python schedule 库 |

### 4. 下一步扩展方向

- **对接更多 MCP 服务**：接入文件系统、数据库、Slack、GitHub 等 MCP 服务器
- **Web UI**：使用 Gradio 或 Streamlit 搭建简单的 Web 对话界面
