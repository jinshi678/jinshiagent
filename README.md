...
>
> **当前版本**：[v0.4.0](https://github.com/jinshi678/jinshiagent/releases/tag/v0.4.0) · [更新日志](CHANGELOG.md)
...
│   └── test_mcp.py             # MCP 协议模块测试（v0.4.0 新增）
├── docs/                         # 项目文档
...
│   ├── mcp_demo.py             # MCP 协议演示（v0.4.0 新增）
│   └── e2e_demo.py             # 端到端完整流程演示（v0.4.0 新增）
...
# 3. 适配到 ToolRegistry（Agent 可透明调用）
registry = ToolRegistry()
...
# 5. 也可通过 Agent ReAct 循环自动调用
agent = Agent(name="mcp-agent", llm_client=llm, tool_registry=registry)
answer = agent.run("请查询北京天气")  # 自动通过 MCP 调用远程工具
...
| 组件 | 类 | 功能 |
|------|------|------|
| MCP 客户端 | `MCPClient` | 连接 MCP 服务器、发现和调用工具 |
| 传输层 | `StdioTransport` / `HTTPTransport` | stdio 子进程 / HTTP SSE 通信 |
| 工具适配器 | `MCPToolAdapter` | 将 MCP 工具桥接到 ToolRegistry |
...
client = MCPClient(config)
tools = await client.connect()  # 自动握手 + 工具发现
result = await client.call_tool("search", {"query": "AI"})  # 调用远程工具
```
