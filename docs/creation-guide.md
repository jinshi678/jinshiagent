...
| `/标题 <平台> <主题>` | — | 仅生成标题 |
| `/脚本 <平台> <主题>` | — | 仅生成视频脚本（画面+台词） |
| `/口播 <平台> <主题>` | `/旁白` | 仅生成口播脚本（纯旁白/单人出镜） |
| `/分镜 <平台> <主题>` | `/镜头` | 仅生成分镜脚本（含镜头设计） |
| `/钩子 <平台> <主题>` | `/开头` | 仅生成爆款开头钩子（前3秒） |
| `/文案 <平台> <主题>` | — | 仅生成文案 |
| `/标签 <平台> <主题>` | — | 仅生成标签策略 |
| `/封面 <平台> <主题>` | — | 仅生成封面文案 |
...
`template_type` 可选值：`title`(标题）、`script`(脚本/画面+台词）、`voice_over`(口播/纯旁白）、`storyboard`(分镜/镜头设计）、`hook`(钩子/开头3秒）、`copywriting`(文案）、`tags`(标签）、`cover`(封面）
...
| | 脚本模板 (`/脚本`) | 口播脚本 (`/口播`) |
|---|---|---|
| 适用场景 | 多人出镜、有画面设计需求 | 单人出镜、纯口播视频 |
| 内容形式 | 画面描述 + 台词/字幕 | 纯口播文本 + 语气提示 |
| 镜头设计 | 含景别、运镜建议 | 无画面指示 |
| 适用平台 | 抖音、B站、快手 | 抖音、视频号、小红书视频笔记 |

**建议**：有拍摄团队用 `/脚本`，个人博主用 `/口播`。
...
短视频**前 3 秒**决定用户是否继续看下去。钩子文案的核心是制造**信息缺口**——让观众想知道「后面发生了什么」。

**5 种钩子类型：**

1. **悬念型**：「你绝对不知道…」
2. **冲突型**：「XXX？大错特错！」
3. **反常识型**：「大家都这样做，其实完全错了」
4. **提问型**：「XXX 你中了几条？」
5. **利益型**：「3 秒学会 XXX，效果太绝了」

**使用建议**：用 `/钩子` 生成 5 个选项 → 挑选最贴合内容的 → 放入 `/脚本` 或 `/口播` 的开头。

---

## v0.5.2 更新亮点

- 🆕 **口播脚本**：专为单人出镜视频设计，纯口播文本+语气提示
- 🆕 **分镜脚本**：完整镜头设计，含景别/运镜/时长/BGM
- 🆕 **爆款钩子**：5 种钩子公式，决定前 3 秒完播率
- 📋 **指令完善**：`/口播` `/分镜` `/钩子` 三条新指令
- 📖 **文档完善**：完整使用教程 + 高频指令速查
